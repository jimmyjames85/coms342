...
public class Evaluator implements Visitor<Value, Env<Value>>
{
	...
	@Override
	public Value visit(LetExp e, Env<Value> env)
	{ // New for varlang.
	...
	@Override
	public Value visit(DefineDecl e, Env<Value> env)
	{ // New for definelang.
		String name = e.name();
	...
	public Value visit(CdrExp e, Env<Value> env)
	{
		...
		{
			if (result instanceof Value.EmptyList)
				return result;
	...
	{ // New for funclang.
		...
		for (int i = length - 1; i > 0; i--)
		{
			if (list == null)
				...
				list = new PairVal(elems.get(i), list);
		}
		if (list == null)
			list = new ExtendList(elems.get(0), new Value.EmptyList());
		else
			list = new ExtendList(elems.get(0), list);
		return list;
	}
	...
	@Override
	public Value visit(LetrecExp e, Env<Value> env)
	{ // New for reclang.
		List<String> names = e.names();
