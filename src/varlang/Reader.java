...
public class Reader
{
	...
	private static final int program = 0, exp = 1, varexp = 2, numexp = 3, addexp = 4, subexp = 5, multexp = 6, divexp = 7, letexp = 8, leteexp = 9, decexp = 10;// New expression for varlang.
	...
	class TreeToExpConverter implements ParseTreeVisitor<AST.Exp>
	{
		...
		public AST.Exp visitChildren(RuleNode node)
		{
			switch (node.getRuleContext().getRuleIndex())
			{
				...
				case leteexp:
					return convertLeteExp(node);
				case decexp:
					return convertDecExp(node);
				case exp:
		...
		/**
		 * Syntax: dec numexp Identifier
		 */
		private AST.Exp convertDecExp(RuleNode node)
		{
			int index = expect(node, 0, "(", "dec");//, "(");
			String sNum=node.getChild(index++).getText();
			int code=0; 
			try
			{
				code= Integer.parseInt(sNum);
			}
			catch (NumberFormatException e)
			{
				throw new ConversionException("Conversion error: " + node.toStringTree(parser) + ", " + "expected Number, found " + node.getChild(index-1).toStringTree(parser));
			}
			
			if (node.getChildCount() > 5)
				throw new ConversionException("Conversion error: " + node.toStringTree(parser) + ", " + "expected only decexp, found " + node.getChildCount() + " nodes.");
			
			String s = node.getChild(index).getText();
			AST.VarExp varExp = new AST.VarExp(s);

			AST.SubExp decExp = new AST.SubExp(varExp);
			decExp.add(new AST.Const(code)); 
			return decExp;
		}		
		...
		/**
		 * Syntax: (lete numexp ((name value_exp)* ) body)
		 */
		private AST.Exp convertLeteExp(RuleNode node)
		{
			int index = expect(node, 0, "(", "lete");//, "(");
			String sNum=node.getChild(index++).getText();
			int code=0; 
			try
			{
			...
			while (match(node, index, "("))
			{
				...
				AST.AddExp addCodeExp = new AST.AddExp(value_exp);
				addCodeExp.add(new AST.Const(code));

				value_exps.add(addCodeExp);
				index = expect(node, index, ")");
			}
			expect(node, index++, ")");
			AST.Exp body = node.getChild(index++).accept(this);
