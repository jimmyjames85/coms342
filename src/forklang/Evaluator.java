...
public class Evaluator implements Visitor<Value>
{
	...
	{ // New for funclang.
		return new Value.FunVal(env, e.formals(), e.body(), e.defaultExp());
	}
	...
	{ // New for funclang.
		...
		if (formals.size() != actuals.size())
			actuals.add((Value) operator.defaultExp().accept(this, env));
	...
	public Value visit(FreqExp e, Env env)
	{ // New for reflang.
		Exp loc_exp = e.loc_exp();
		Value.RefVal loc = (Value.RefVal) loc_exp.accept(this, env);
		
		return heap.frequency(loc);
	...
	{ // New for reflang.
		...
		Value currentVal = heap.deref(loc);
		
		Value assign_val; 
		if(currentVal.getClass().isAssignableFrom(rhs_val.getClass()))
			assign_val= heap.setref(loc, rhs_val);
		else
		{
			Printer.Formatter f = new Printer.Formatter();
			String setexp=f.visit(e, env);
			
			
			
			assign_val= new DynamicError("Assigning a value of an incompatible type to the location in " + setexp);
		}
	...
	private Env initialEnv()
	{
		...
		Value.FunVal readFun = new Value.FunVal(initEnv, formals, body, new AST.Const(3.14));
		...
		Value.FunVal requireFun = new Value.FunVal(initEnv, formals, body, new AST.Const(3.14));
