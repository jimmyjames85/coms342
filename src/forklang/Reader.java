...
public class Reader
{
	...
	private static final int program = 0, definedecl = 1, exp = 2, varexp = 3, numexp = 4, strconst = 5, boolconst = 6, addexp = 7, subexp = 8, multexp = 9, divexp = 10,
			...
			forkexp = 28, lockexp = 29, unlockexp = 30, freqexp = 31;
	...
	class TreeToExpConverter implements ParseTreeVisitor<AST.Exp>
	{
		...
		private AST.Exp convertLambdaExp(RuleNode node)
		{
			...
			AST.Exp defaultValue = null;
			while (!match(node, index, ")"))
			{
				String formal = expectString(node, index++, 1)[0];
				if(formal.equals("("))
				{
					formal = expectString(node, index++, 1)[0];
					formals.add(formal);
					index = expect(node, index++, "=");
					defaultValue = expectExp(node, index++, 1)[0];
					index = expect(node, index++, ")");					
				}
				else
					formals.add(formal);
			...
			return new AST.LambdaExp(formals, body_exp, defaultValue);
		}
		...
		private AST.Exp convertFreqExp(RuleNode node)
		{
			System.out.println("hello?");
			int index = expect(node, 0, "(", "frequency");
			AST.Exp loc_exp = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			return new AST.FreqExp(loc_exp);
		}
