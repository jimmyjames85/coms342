...
public class Reader {
	...
	private static String GRAMMAR_FILE = "src/reactlang/ReactLang.g";
	...
	private static final int 
		...
		signalexp = 34, readexp = 35, writeexp = 36
		;
	...
	class TreeToExpConverter implements ParseTreeVisitor<AST.Exp> {
		...
		public AST.Exp visitChildren(RuleNode node) {			
			switch(node.getRuleContext().getRuleIndex()){
				...
				case letrecexp: return convertLetrecExp(node);
                                ...
                                case lockexp: return convertLockExp(node);
                                case unlockexp: return convertUnlockExp(node);
                                case eventexp: return convertEventExp(node);
                                case announceexp: return convertAnnounceExp(node);
                                case whenexp: return convertWhenExp(node);
                                case signalexp: return convertSignalExp(node);
                                case readexp: return convertReadSigExp(node);
                                case writeexp: return convertWriteSigExp(node);
		...
		private AST.Exp convertWhenExp(RuleNode node){
			...
			AST.Exp body = node.getChild(index++).accept(this);
			expect(node,index++, ")");
			return new AST.WhenExp(event, body);
		}
		
		private AST.Exp convertSignalExp(RuleNode node){
			int index = expect(node,0,"(", "signal");
			AST.Exp val = node.getChild(index++).accept(this);
			expect(node,index++, ")");
			return new AST.SignalExp(val);
		}
		
		private AST.Exp convertReadSigExp(RuleNode node){
			int index = expect(node,0,"(", "read");
			AST.Exp signal_exp = node.getChild(index++).accept(this);
			expect(node,index++, ")");
			return new AST.ReadSigExp(signal_exp);
		}

		private AST.Exp convertWriteSigExp(RuleNode node){
			int index = expect(node,0,"(", "write");
			AST.Exp signal_exp = node.getChild(index++).accept(this);
			AST.Exp val_exp = node.getChild(index++).accept(this);
			expect(node,index++, ")");
			return new AST.WriteSigExp(signal_exp, val_exp);
		}
...
//    				AST.Exp event = expectExp(node, index++, 1)[0];
        			...
        			expect(node,index++, ")");
        			return new AST.AnnounceExp(event, actuals);
                }
