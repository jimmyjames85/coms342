package reactlang;
import static reactlang.AST.*;
import static reactlang.Heap.*;
import static reactlang.Value.*;

import java.util.List;
import java.util.ArrayList;
import java.io.File;

import reactlang.AST.AnnounceExp;
import reactlang.AST.EventExp;
import reactlang.AST.ReadSigExp;
import reactlang.AST.SignalExp;
import reactlang.AST.WhenExp;
...
//			Value val = (Value) exp.accept(this, env); // Dynamic type-checking
//			if(val instanceof SignalVal)
//				intermediate = (NumVal)((SignalVal) val).value();
//			else
//				intermediate = (NumVal) val;
	...
	@Override
	public Value visit(EventExp e, Env env) {
		return new Value.EventVal(e.contexts());
	}

	@Override
	public Value visit(AnnounceExp e, Env env) {
        Exp event_exp = e.event();
        Object result = event_exp.accept(this, env);
		if(!(result instanceof Value.EventVal))
			return new Value.DynamicError("Non-event value cannot be announced in expression " +  ts.visit(e, env));
        Value.EventVal event = (Value.EventVal) result;
		if(!(e.actuals().size()== event.contexts().size()))
			return new Value.DynamicError("Number of context varaibles do not match in announce expression " +  ts.visit(e, env));
		...
		Env ann_env = appendEnv(env, initEnv);
	...
	@Override
	public Value visit(WhenExp e, Env env) {
        Exp event_exp = e.event();
        Object result = event_exp.accept(this, env);
		if(!(result instanceof Value.EventVal))
			return new Value.DynamicError("Non-event value cannot be used in when expression " +  ts.visit(e, env));
	...
	@Override
	public Value visit(SignalExp e, Env env) {
		Exp value_exp = e.value_exp();
		return new Value.SignalVal(value_exp, env, this, heap);
	}

	@Override
	public Value visit(ReadSigExp e, Env env) {
        Exp signal_exp = e.signal_exp();
        Object result = signal_exp.accept(this, env);
		if(!(result instanceof Value.SignalVal))
			return new Value.DynamicError("Non-signal value cannot be used in read expression " +  ts.visit(e, env));
		SignalVal signal = (SignalVal) result;
		return signal.read();
	}
	 ...
	 */
	@Override
	public Value visit(WriteSigExp e, Env env) {
        Exp signal_exp = e.signal_exp();
        Object result = signal_exp.accept(this, env);
		if(!(result instanceof Value.SignalVal))
			return new Value.DynamicError("Non-signal value cannot be used in write expression " +  ts.visit(e, env));
		SignalVal signal = (SignalVal) result;
		Exp val_exp = e.val_exp();
		signal.write(val_exp, env);
		return (Value) val_exp.accept(this, env);
	}
