...
public class Reader {
	...
	private static final int 
		...
		forkexp = 28, lockexp = 29, unlockexp = 30, synchexp = 31
		;
	...
	class TreeToExpConverter implements ParseTreeVisitor<AST.Exp> {
		...
		public AST.Exp visitChildren(RuleNode node) {			
			switch(node.getRuleContext().getRuleIndex()){
				...
				case letrecexp: return convertLetrecExp(node);
                                ...
                                case synchexp: return convertSynchExp(node);
		...
		private AST.Exp convertLetrecExp(RuleNode node){
			...
			return new AST.LetrecExp(names,value_exps,body);
                ...
                private AST.Exp convertSynchExp(RuleNode node)
                {
                	 int index = expect(node,0,"(", "synchronized");
                     AST.Exp lock= node.getChild(index++).accept(this);
                     AST.Exp eval_exp = node.getChild(index++).accept(this);
                     expect(node,index++, ")");
                     return new AST.SynchExp(lock, eval_exp);                	
                }
