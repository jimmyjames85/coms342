...
public class Evaluator implements Visitor<Value>
{
	...
	@Override
	public Value visit(SynchExp e, Env env)
	{
		Exp lock= e.lock();
		Value lockVal = (Value)lock.accept(this, env);
		if (!(lockVal instanceof Value.RefVal))
			return new Value.DynamicError("Non-reference values cannot be locked in expression " + ts.visit(e, env));
		
		Value.RefVal refVal= (Value.RefVal)lockVal;
		refVal.lock();

		Exp eval_exp = e.eval_exp();
		Value result = (Value) eval_exp.accept(this, env);
