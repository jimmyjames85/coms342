...
public class Evaluator implements Visitor<Value>
{
	...
	public Value visit(AnnounceExp e, Env env)
	{
		...
		List<Exp> observers = event.observers();
		
		if(e.priorityIsHigh)
			java.util.Collections.reverse(observers);

		Value lastVal = new UnitVal();
		
		List<String> listval = new ArrayList<String>();
		
		for (Exp exp : observers)
		{
			lastVal = (Value) exp.accept(this, ann_env);
			listval.add(lastVal.tostring());
		}
		
		System.out.println("Anounce called: " + listval.toString());		
	...
	public Value visit(WhenExp e, Env env)
	{
		...
		Exp priority_exp = e.priority();
		Object priority = priority_exp.accept(this, env);

		if (!(priority instanceof Value.NumVal))
			return new Value.DynamicError("Priority Expressions did not evaluate to a NumVal: Sorry dooood :-(");

		EventVal event = (EventVal) result;
		Exp observer = e.body();
		event.register(observer, (NumVal) priority);

		return new UnitVal();
	}
	
	public Value visit(UnregisterExp e, Env env)
	{
		...
		Exp priority_exp = e.priority();
		result= priority_exp.accept(this, env);

		if (!(result instanceof Value.NumVal))
			return new Value.DynamicError("Priority Expressions did not evaluate to a NumVal: Sorry dooood :-(");
		double priority = ((NumVal)result).v();
