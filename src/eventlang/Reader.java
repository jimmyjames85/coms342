...
public class Reader
{
	...
	private static String GRAMMAR_FILE = "src/eventlang/EventLang.g";
	...
	private static final int program = 0, definedecl = 1, exp = 2, varexp = 3, numexp = 4, strconst = 5, boolconst = 6, addexp = 7, subexp = 8, multexp = 9, divexp = 10,
			...
			forkexp = 28, lockexp = 29, unlockexp = 30, eventexp = 31, announceexp = 32, whenexp = 33, // New expression for the eventlang language.
			unregisterexp = 34;
	...
	class TreeToExpConverter implements ParseTreeVisitor<AST.Exp>
	{
		...
		public AST.Exp visitChildren(RuleNode node)
		{
			switch (node.getRuleContext().getRuleIndex())
			{
				...
				case strconst:
					return convertStrConst(node);
				case boolconst:
					return convertBoolConst(node);
				case addexp:
					return convertAddExp(node);
				case subexp:
					return convertSubExp(node);
				case multexp:
					return convertMultExp(node);
				case divexp:
					return convertDivExp(node);
				case letexp:
					return convertLetExp(node);
				case definedecl:
					return convertDefineDecl(node);
				case lambdaexp:
					return convertLambdaExp(node);
				case callexp:
					return convertCallExp(node);
				case ifexp:
					return convertIfExp(node);
				case lessexp:
					return convertLessExp(node);
				case equalexp:
					return convertEqualExp(node);
				case greaterexp:
					return convertGreaterExp(node);
				case carexp:
					return convertCarExp(node);
				case cdrexp:
					return convertCdrExp(node);
				case consexp:
					return convertConsExp(node);
				case listexp:
					return convertListExp(node);
				case nullexp:
					return convertNullExp(node);
				case letrecexp:
					return convertLetrecExp(node);
				case refexp:
					return convertRefExp(node);
				case derefexp:
					return convertDerefExp(node);
				case assignexp:
					return convertAssignExp(node);
				case freeexp:
					return convertFreeExp(node);
				case forkexp:
					return convertForkExp(node);
				case lockexp:
					return convertLockExp(node);
				case unlockexp:
					return convertUnlockExp(node);
				case eventexp:
					return convertEventExp(node);
				case announceexp:
					return convertAnnounceExp(node);
				case whenexp:
					return convertWhenExp(node);
				case unregisterexp:
					return convertUnregisterExp(node);
				case exp:
		...
		private AST.Const convertConst(RuleNode node)
		{
			...
			{
				throw new ConversionException("Conversion error: " + node.toStringTree(parser) + ", " + "expected Number, found " + node.getChild(0).toStringTree(parser));
			}
		}
		...
		private AST.Exp convertWhenExp(RuleNode node)
		{
			int index = expect(node, 0, "(", "when");
			AST.Exp event = node.getChild(index++).accept(this);
			expect(node, index++, "do");
			AST.Exp body = node.getChild(index++).accept(this);
			AST.Exp priority = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			AST.WhenExp ret = new AST.WhenExp(event, body, priority);
			return ret;
		}

		private AST.Exp convertUnregisterExp(RuleNode node)
		{
			int index = expect(node, 0, "(", "unregister");
			AST.Exp event = node.getChild(index++).accept(this);
			AST.Exp priority = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			AST.UnregisterExp ret = new AST.UnregisterExp(event, priority);
			return ret;
		...
		{
			int index = expect(node, 0, "(", "lock");
			AST.Exp value_exp = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			return new AST.LockExp(value_exp);
		}

		private AST.Exp convertUnlockExp(RuleNode node)
		{
			int index = expect(node, 0, "(", "unlock");
			AST.Exp value_exp = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			return new AST.UnlockExp(value_exp);
		}

		private AST.Exp convertEventExp(RuleNode node)
		{
			int index = expect(node, 0, "(", "event");
			//    				String name = expectString(node, index++, 1)[0];
			expect(node, index++, "(");
			List<String> contexts = new ArrayList<String>();
			while (!match(node, index, ")"))
			{
				String context = expectString(node, index++, 1)[0];
				contexts.add(context);
			}
			expect(node, index++, ")");
			//return new AST.EventExp(name, contexts);
			return new AST.EventExp(contexts);
		}

		private AST.Exp convertAnnounceExp(RuleNode node)
		{
			int index = expect(node, 0, "(", "announce");
			//String event = expectString(node, index++, 1)[0];
			//    				AST.Exp event = expectExp(node, index++, 1)[0];
			AST.Exp event = node.getChild(index++).accept(this);
			expect(node, index++, "(");
			List<AST.Exp> actuals = new ArrayList<AST.Exp>();
			while (!match(node, index, ")"))
			{
			...
			boolean high = match(node, index, "High");
			boolean low = match(node, index, "Low");

			return new AST.AnnounceExp(event, actuals, high);
		}
		 ...
		 * @return an array of n + numTokens expressions, if expectations is
		 *         met. Otherwise, throws ConversionException.
		 */
		protected AST.Exp[] expectExp(RuleNode node, int n, int numTokens)
		{
			...
			for (int i = 0; i < numTokens; i++)
			{
				...
				if (value == null || value instanceof AST.ErrorExp)
					throw new ConversionException("Conversion error: " + node.toStringTree(parser) + ", " + "expected Exp, found " + node.getChild(n + i).toStringTree(parser));
				results[i] = value;
			}
			return results;
		}
		 ...
		 * @return an array of n + numTokens strings, if expectations is met.
		 *         Otherwise, throws ConversionException.
		 */
		protected String[] expectString(RuleNode node, int n, int numTokens)
		{
			...
			for (int i = 0; i < numTokens; i++, n++)
			{
				...
				if (value == null || node.getChild(n).getChildCount() != 0)
					throw new ConversionException("Conversion error: " + node.toStringTree(parser) + ", " + "expected Exp, found " + node.getChild(n).toStringTree(parser));
				results[i] = value;
			}
			return results;
		}
		 ...
		 * @param tokens
		 *            - expected strings.
		 * @return n + tokens.length, if expectations is met. Otherwise, throws
		 *         ConversionException.
		 */
		protected int expect(RuleNode node, int n, String... tokens)
		{
			...
			for (int i = 0; i < numTokens; i++)
			{
				if (!node.getChild(n + i).toStringTree(parser).equals(tokens[i]))
					throw new ConversionException("Conversion error: " + node.toStringTree(parser) + ", " + "expected " + tokens[i] + ", found "
