...
public class Reader
{
	...
	private static final int program = 0, definedecl = 1, exp = 2, varexp = 3, numexp = 4, strconst = 5, boolconst = 6, addexp = 7, subexp = 8, multexp = 9, divexp = 10,
			letexp = 11, lambdaexp = 12, callexp = 13, ifexp = 14, lessexp = 15, equalexp = 16, greaterexp = 17, carexp = 18, cdrexp = 19, consexp = 20, listexp = 21,
			nullexp = 22, letrecexp = 23, refexp = 24, derefexp = 25, assignexp = 26, freeexp = 27, type = 28, booltype = 29, funtype = 30, numtype = 31, listtype = 32,
	...
	{
		String programText = "(require \"" + fileLoc + "\")";
		Program program = parse(programText);
		return program;
	}
	...
	class TreeToExpConverter implements ParseTreeVisitor<AST.Exp>
	{
		...
		private AST.Exp convertDerefExp(RuleNode node)
		{
			...
			expect(node, index++, ")");
			return new AST.DerefExp(loc_exp, type);
		}
	 ...
	 *         ConversionException.
	 */
	protected int staticexpect(ParserInterpreter parser, RuleNode node, int n, String... tokens)
	{
	 ...
	 */
	public static boolean staticmatch(ParserInterpreter parser, RuleNode node, int n, String... tokens)
	{
		int numTokens = tokens.length;
		for (int i = 0; i < numTokens; i++)
		{
	...
	public class TreeToTypeConverter implements ParseTreeVisitor<Type>
	{
		...
		{
			System.out.println("TreeToTypeConverter visit: " + tree.toStringTree(parser));
			return null;
		}
		...
		public Type visitChildren(RuleNode node)
		{
			switch (node.getRuleContext().getRuleIndex())
			{
				...
				case funtype:
					return getFunType(node);
				case numtype:
					return NumT.getInstance();
				case listtype:
					return getListType(node);
				case pairtype:
					return getPairType(node);
				case reftype:
					return getRefType(node);
				case stringtype:
					return StringT.getInstance();
				case unittype:
					return UnitT.getInstance();
				case type:
					return visitChildrenHelper(node);
				default:
					System.out.println("Conversion error (from parse tree to AST):" + " found unknown/unhandled case "
