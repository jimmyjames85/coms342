...
public class Checker implements Visitor<Type, Env<Type>>
{
	...
	public Type visit(LetExp e, Env<Type> env)
	{
		...
		for (Exp exp : value_exps)
		{
			...
			{
				return new ErrorT("The declared type of the " + i + " let variable and the actual type mismatch, expect " + argType.tostring() + " found " + type.tostring()
						+ " in " + ts.visit(e, null));
			}
	...
	public Type visit(LambdaExp e, Env<Type> env)
	{
		...
		if (type instanceof FuncT)
		{
			...
			message = "The number of formal parameters and the number of " + "arguments in the function type do not match in ";
			if (types.size() == names.size())
			{
				...
				message = "The declared return type and the actual return type " + "mismatch, expect " + type.tostring() + ", but found " + bodyType.tostring();
		...
		if (type instanceof FuncT)
		{
			...
			if (size_actuals == size_formals)
			{
				for (int i = 0; i < size_actuals; i++)
				{
					...
					{
						return new ErrorT("The expected type of the " + i + " argument is " + argTypes.get(i).tostring() + " found " + operand_type.tostring() + " in "
								+ ts.visit(e, null));
					}
				}
	...
	public Type visit(IfExp e, Env<Type> env)
	{
		...
		return new ErrorT("The then and else expressions should have the same " + "type, then has type " + thentype.tostring() + " else has type " + elsetype.tostring() + " in "
				+ ts.visit(e, null));
	}
	...
	public Type visit(NullExp e, Env<Type> env)
	{
		...
		return new ErrorT("The null? expect an expression of type List, found " + type.tostring() + " in " + ts.visit(e, null));
	}
	...
	public Type visit(RefExp e, Env<Type> env)
	{
		...
		return new ErrorT("The Ref expression expect type " + type.tostring() + " found " + expType.tostring() + " in " + ts.visit(e, null));
	}
	...
	public Type visit(DerefExp e, Env<Type> env)
	{
		...
		if (expType instanceof RefT )
		{
			RefT rt = (RefT) expType;


			if(type.typeEqual(rt._nestType))
				return rt.nestType();
		}

		return new ErrorT("The dereference expression expect a reference type " + "found " + expType.tostring() + " in " + ts.visit(e, null));
	}
	...
	public Type visit(AssignExp e, Env<Type> env)
	{
		...
		return new ErrorT("The lhs of the assignment expression expect a " + "reference type found " + lhsType.tostring() + " in " + ts.visit(e, null));
	}
	...
	public Type visit(FreeExp e, Env<Type> env)
	{
		...
		return new ErrorT("The free expression expect a reference type " + "found " + type.tostring() + " in " + ts.visit(e, null));
	}
