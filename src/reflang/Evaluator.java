...
public class Evaluator implements Visitor<Value>
{
	...
	public Value visit(VarExp e, Env env)
	{
		...
		return heap.deref(env.get(e.name()));
	}
	...
	{ // New for varlang.
		...
		List<RefVal> refValues = new ArrayList<RefVal>(value_exps.size());
		
		for (Exp exp : value_exps)
			refValues.add((RefVal)heap.ref((Value)exp.accept(this, env)));
		//values.add((Value) exp.accept(this, env));

		Env new_env = env;
		for (int index = 0; index < names.size(); index++)
			new_env = new ExtendEnv(new_env, names.get(index), refValues.get(index));
	...
	{ // New for definelang.
		...
		initEnv = new ExtendEnv(initEnv, name, ((RefVal)(heap.ref(value))));
	...
	{ // New for funclang.
		...
		for (int index = 0; index < formals.size(); index++)
			fun_env = new ExtendEnv(fun_env, formals.get(index), (RefVal)(heap.ref(actuals.get(index))));
	...
	{
		Heap initHeap = new Heap.Heap16Bit();
		...
		initEnv = new Env.ExtendEnv(initEnv, "read", (RefVal)initHeap.ref(readFun));
		...
		initEnv = new Env.ExtendEnv(initEnv, "require", (RefVal)(initHeap.ref(requireFun)));
