...
import funclangHw5.AST.*;

public class Reader
{

	private static String GRAMMAR_FILE = "build/funclangHw5/FuncLang.g";
	...
	private static final int program = 0, definedecl = 1, exp = 2, varexp = 3, numexp = 4, strconst = 5, boolconst = 6, addexp = 7, subexp = 8, multexp = 9, divexp = 10,
			...
			numQexp=23, boolQexp=24, stringQexp=25, procQexp=26, pairQexp=27, listQexp=28, nullQexp=29, unitQexp=30;
	...
	class TreeToExpConverter implements ParseTreeVisitor<AST.Exp>
	{
		...
		public AST.Exp visitChildren(RuleNode node)
		{
			switch (node.getRuleContext().getRuleIndex())
			{
				...
				case numQexp:
					return convertNumQExp(node);
				case boolQexp:
					return convertBoolQexp(node);
				case stringQexp:
					return convertStringQexp(node);
				case procQexp:
					return convertProcQexp(node);
				case pairQexp:
					return convertPairQexp(node);
				case listQexp:
					return convertListQexp(node);
				case nullQexp:
					return convertNullQexp(node);
				case unitQexp:
					return convertUnitQexp(node);
				case exp:
		...
		private AST.Exp convertNumQExp(RuleNode node)
		{
			int index = expect(node, 0, "(", "number?");
			AST.Exp _exp = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			return new AST.NumQExp(_exp);			
		}

		private AST.Exp convertBoolQexp(RuleNode node)
		{
			int index = expect(node, 0, "(", "boolean?");
			AST.Exp _exp = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			return new AST.BoolQExp(_exp);			
		}

		private AST.Exp convertStringQexp(RuleNode node)
		{
			int index = expect(node, 0, "(", "string?");
			AST.Exp _exp = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			return new AST.StringQExp(_exp);			
		}

		private AST.Exp convertProcQexp(RuleNode node)
		{
			int index = expect(node, 0, "(", "procedure?");
			AST.Exp _exp = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			return new AST.ProcedureQExp(_exp);			
		}
		

		private AST.Exp convertPairQexp(RuleNode node)
		{
			int index = expect(node, 0, "(", "pair?");
			AST.Exp _exp = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			return new AST.PairQExp(_exp);			
		}

		private AST.Exp convertListQexp(RuleNode node)
		{
			int index = expect(node, 0, "(", "list?");
			AST.Exp _exp = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			return new AST.ListQExp(_exp);			
		}

		private AST.Exp convertNullQexp(RuleNode node)
		{
			int index = expect(node, 0, "(", "null?");
			AST.Exp _exp = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			return new AST.NullQExp(_exp);			
		}

		private AST.Exp convertUnitQexp(RuleNode node)
		{
			int index = expect(node, 0, "(", "unit?");
			AST.Exp _exp = node.getChild(index++).accept(this);
			expect(node, index++, ")");
			return new AST.UnitQExp(_exp);			
		}
	...
	private String runFile(String programText) throws IOException
	{
		...
		{
			programText = readFile("build/funclangHw5/" + programText.substring(4));
		}
		return programText;
	}
}
