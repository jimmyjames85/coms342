...
import static funclangHw5.AST.*;
import static funclangHw5.Value.*;

import java.util.List;
import java.util.ArrayList;
import java.io.File;

import funclangHw5.Env.*;
...
public class Evaluator implements Visitor<Value>
{
	...
	public Value visit(NumQExp e, Env env)
	{
		Value val = (Value) e.arg().accept(this, env);
		return new BoolVal(val instanceof Value.NumVal);
	}

	public Value visit(AST.BoolQExp e, Env env)
	{
		Value val = (Value) e.arg().accept(this, env);
		return new BoolVal(val instanceof Value.BoolVal);
	}

	public Value visit(AST.StringQExp e, Env env)
	{
		Value val = (Value) e.arg().accept(this, env);
		return new BoolVal(val instanceof Value.StringVal);
	}

	public Value visit(AST.ProcedureQExp e, Env env)
	{
		Value val = (Value) e.arg().accept(this, env);
		return new BoolVal(val instanceof Value.FunVal);
	}

	public Value visit(AST.PairQExp e, Env env)
	{
		Value val = (Value) e.arg().accept(this, env);
		return new BoolVal(val instanceof Value.PairVal);
	}

	private boolean isList(PairVal val)
	{
		if (val._snd instanceof Value.Null)
			return true;
		if (val._snd instanceof Value.PairVal && isList((Value.PairVal) val._snd))
			return true;
		System.out.println(val._snd.getClass());
		return false;
	}

	public Value visit(AST.ListQExp e, Env env)
	{
		Value val = (Value) e.arg().accept(this, env);
		return new BoolVal(val instanceof Value.Null || ( val instanceof PairVal && isList((PairVal) val)));
	}

	public Value visit(AST.NullQExp e, Env env)
	{
		Value val = (Value) e.arg().accept(this, env);
		return new BoolVal(val instanceof Value.Null);
	}

	public Value visit(AST.UnitQExp e, Env env)
	{
		Value val = (Value) e.arg().accept(this, env);
