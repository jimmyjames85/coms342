...
public class Evaluator implements Visitor<Value>
{
	...
	{
		List<Exp> expList = e.elems();
		List<EvalThread> threadList = new ArrayList<Evaluator.EvalThread>();
		
		for(Exp curExp : expList)
		{
			EvalThread newThread = new EvalThread(env, curExp, this);
			newThread.start();
		...
		Value result = new Value.Null();
		for (int i = length - 1; i >= 0; i--)
			result = new PairVal(threadList.get(i).value(), result);

		return result;
/*		
		for(EvalThread eThread)
		Exp fst = e.fst_exp();
		Exp snd = e.snd_exp();
