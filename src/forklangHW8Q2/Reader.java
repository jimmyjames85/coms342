...
public class Reader
{
	...
	class TreeToExpConverter implements ParseTreeVisitor<AST.Exp>
	{
		...
		private AST.Exp convertForkExp(RuleNode node)
		{
			...
			ArrayList<AST.Exp> expList = new ArrayList<AST.Exp>();
			
			//forkExp needs at least two exps
			expList.add(node.getChild(index++).accept(this));
			expList.add(node.getChild(index++).accept(this));
			...
			expect(node,index++, ")");
			return new AST.ForkExp(expList);
		}
