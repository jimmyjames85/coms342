...
public class Evaluator implements Visitor<Value>
{
	...
	public Value visit(ForkExp e, Env env)
	{
		List<Exp> operands = e.getOperands();

		List<EvalThread> threads = new ArrayList<Evaluator.EvalThread>();

		for (Exp curExp : operands)
			threads.add(new EvalThread(env, curExp, this));
		...
		for(EvalThread thread : threads)
			vals.add(thread.value());

		
		PairVal head = new PairVal(vals.remove(0), new Null());

		PairVal next=head;
		while(!vals.isEmpty())
		{
			Value val = vals.remove(0);
			next._snd =new PairVal(val,  new Null());
